...
This project is based on https://github.com/svfranklindemo/flosi-refdemo-18062026-132642, itself derived from the https://github.com/adobe/aem-boilerplate/ boilerplate. It is a WKND Universal Demo showcasing AEM capabilities including Dynamic Media, Content Fragments, Adaptive Forms, and custom themes. You are expected to follow the coding style and practices established in the boilerplate, but add functionality according to the needs of the site currently developed.
...
**We do NOT run a local dev server.** All changes are committed directly to the `main` branch and tested on the cloud preview environment:

- **Preview**: https://main--flosi-refdemo-18062026-132642--svfranklindemo.aem.page/
- **Live**: https://main--flosi-refdemo-18062026-132642--svfranklindemo.aem.live/
- **AEM Cloud Author**: https://author-p165802-e1765367.adobeaemcloud.com

After every commit to `main`, AEM Code Sync automatically deploys to preview within seconds. The user will test there directly.

## AEM Content Paths

Content is served from AEM Cloud. The `paths.json` maps AEM paths to URL paths:
- Site root: `/content/6a33f2107a364920b164142a/language-masters/en/` → `/`
- Languages: en, fr, es, ja, de
- DAM: `/content/dam/6a33f2107a364920b164142a/`

## Available Blocks

The following blocks exist in this project (each has a `.js` and `.css` in `blocks/{name}/`):
accordion, cards, carousel, columns, content-fragment, dm-openapi, dynamicmedia-image, dynamicmedia-template, embed-adaptive-form, find-a-doctor, forex, form, fragment, hero, iframe, quote, search, tabs, teaser, video

### Block Themes / Styles (component-models.json)
Key style options available for blocks:
- **Section**: background (bg-default/bg-theme/bg-dark/bg-light), spacing top/bottom, full-width
- **Hero**: layouts (overlay/image-left/image-right/image-background-text-left/image-background-text-right), CTA styles, background styles (default/theme-dark/theme-light)
- **Cards/Card**: styles (image-top/image-bottom/teaser-overlay/teaser-card), CTA styles
- **Teaser**: style (image/video), optional swoosh effect, button properties
- **Page themes**: default, gray-theme, red-theme, blue-theme, green-theme, wehealthcare-theme

## Setup Commands

- Install dependencies: `npm install`
- Start local development (not used in this project — deploy to main instead): `npx -y @adobe/aem-cli up --no-open --forward-browser-logs`
